...
ensure_sanity_bundle() {
	if STEAMOS_WAYDROID_INTERNAL=1 "$DECK_RUNTIME/ensure-bundle-on-deck.sh"; then
		return 0
	fi
...
run_privileged_sanity_checks() {
	...
	if ! printf '%s\n' "$current_password" | sudo -S -k -v >/dev/null 2>&1; then
	...
	if [[ "${REPAIR_MODE:-false}" != true ]] &&
		systemctl is-active --quiet plugin_loader.service; then
		printf 'Temporarily stopping Decky Loader during installation...\n'
		if ! printf '%s\n' "$current_password" |
			sudo -S systemctl stop plugin_loader.service; then
